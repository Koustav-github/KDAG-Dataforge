...
def _index(copied, missing):
    lines = [
        ...
        "| `supporting/` | Primer, deck, and the original spec. Context only — not the artifact. |",
        "| `editable-sources/` | The .docx the PDF was produced from. |",
        "",
        "## Note on `supporting/` — these are historical documents",
        "",
        "`Model_Surgery_Spec.docx` is the **design document written before any code ran**.",
        "It states the hypothesis as a pre-registration — verbatim: *\"Both halves are",
        "falsifiable, and each can fail independently.\"* Both halves subsequently failed.",
        "We have deliberately **not** edited it to match the outcome: a pre-registration",
        "that is revised after seeing the data is worthless as evidence that the hypothesis",
        "came first. Read it as \"what we set out to test\", and `README-project.md` as",
        "\"what happened\".",
        "",
        "The deck and primer are explanatory material and already carry the corrected",
        "findings, including the retracted directional-asymmetry claim.",
        "",
